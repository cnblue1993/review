...
// *****************************************************应急预案，中途专家离场。
		...
		//显示各项目的得票情况。
		public function before_prepared()
		{
		...
		public function prepared(){
			$where=array('s_id'=>$_GET['s_id']);
				// $survey_state=M('survey')->where($where)->select();
				// dump($survey_state[0]['s_state']); die();
			$question=M('question')->where($where)->order('qg_id,q_order,q_id')->select();
			$voter_sum=M('survey')->where($where)->getField('voter_sum');
			foreach($question as $k){
				$middle_result[$k['source_id']]=$this->questionResponseCount($k['source_id']);
				$data[$k['source_id']]=$this->prj_Re_Rank($k['source_id']);
			...
			$this->data=$data;
			$this->list=$question;
			$this->voter_sum=$voter_sum;
			$this->s_id=$_GET['s_id'];
			$s_value=M('survey_attribute')->where('s_id='.$_GET['s_id'])->getField('a_value');
			$this->assign('s_value',$s_value);
			$this->display();
		}
		//状态变换和结果统计更新
			public function prepared2(){
			$data=array('s_id'=>$_GET['sid'],'s_state'=>'评审完毕');
		...
		public function expert_response2(){
			...
			$j=0;
			foreach($expert as $k){
				$where=array('e_name'=>$k['e_name'],'s_id'=>$_GET['sid']);
