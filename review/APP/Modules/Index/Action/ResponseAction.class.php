<?php
	...
	Class ResponseAction extends Action{
		...
		public function saveResponse()
		{
			...
			if($e_state!='评审完毕')
			{
				if($_POST['count']!=($postcount-5)&&$_POST['add'])//防止为未投票直接进入下一页
				{
					$this->error('还有未评审项目！');
				}	
			}
		...
		public function show($page_number)
		{
			...
			$questionlist=M('question')
				...
				->order('q_order,q_id')
				->field('q_id,p_name,p_principal,p_funds,p_rank,p_number,p_id')
				->select();
		...
		public function welcome()
		{
			...
			$expert=M('expert')->where('e_id='.$_SESSION['e_id'])->select();
			$e_state=$expert[0]['e_state'];
			$e_position=$expert[0]['e_position'];
			if($e_state=="评审完毕")
                	$value="查看评审";
                else{
                	$value="开始评审";
            ...
            $this->assign('e_position',$e_position);
			...
			if($_POST)//修改专家状态
			{
                ...
                $page_number=0;
                //$this->redirect(GROUP_NAME.'/Index/frame?page_number='.$page_number);
		...
		//查看个人评审情况
		public function show_question($page)
		{
			$condition=array('info_type'=>'前台title');
			$title=M('site_info')->where($condition)->getField('info_content');
			$this->assign('title',$title);
			...
			$question=M('question')
				->join('tb_question_group on tb_question.qg_id=tb_question_group.qg_id')
				->where('tb_question.s_id='.$_SESSION['s_id'])
				->order('tb_question.qg_id,q_order,q_id')
				->select();
			//dump($question);
			$answer=M('response')->where('voter_id='.$_SESSION['e_id'])->select();
			$this->assign('question',$question);
			$this->assign('answer',$answer);
			//dump($question);
			$this->display();
		}
		public function show_result($page)
		{
			$condition=array('info_type'=>'前台title');
			$title=M('site_info')->where($condition)->getField('info_content');
			$this->assign('title',$title);
			...
			$result=M('question')
				...
				->order('p_department,q_order,q_id')
				->field('p_name,p_department,p_rank,p_result')
				->select();
